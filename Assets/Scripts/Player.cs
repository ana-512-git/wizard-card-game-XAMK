...
public class Player : MonoBehaviour, IDropHandler
{
    ...
    public bool isFire; //wheter an enemy is a fire monster or not
    ...
    {
        if (health>=0 && health< GameController.instance.healhNumbers.Length)
        {
            healthNumberImage.sprite = GameController.instance.healhNumbers[health];
        }
        else
        {
            Debug.LogWarning("Health not a valid number, "+health.ToString());
        }
    }
